...
def all_neighbor_absolute_test(network,
                               dataset,
                               prediction_mode):
    """
    A function that described the MSE and PPE for every different neighbor distance for a
    ...
    prediction_mode : string
        If want to average neighbors uniformly, use 'uniform', it want to apply Boltzmann weighting,
        use 'boltzmann'
    ...
    for curr_test_file in test_file_list:
        avg_result_dict[curr_test_file] = {}
        per_point_result_dict[curr_test_file] = {}
        single_file_avg, single_file_ppe = single_file_neigh_absolute_test(network,
                                                                           dataset,
                                                                           curr_test_file,
                                                                           prediction_mode,
                                                                           num_possible_neighbors)
...
def single_file_committee_via_tis(network_dictionary,
                                  dataset,
                                  curr_test_file):
    """
    ...
    #Models are quite small; can fit all 35 in GPU memory if using workstation!
    for specific_model in network_dictionary:
        curr_network = network_dictionary[specific_model]
        curr_network.eval()
        curr_network.to(device)
    ...
    for possible_point in dataset.test_getitem_list:
        if possible_point[0] == curr_test_file:
            ...
            #Get tensor prediction
            running_point_average = []
            for specific_model in network_dictionary:
                network = network_dictionary[specific_model]
                on_gpu = possible_point[4].to(device)
                try:
                    pred_val = network(on_gpu).item()
                except:
                    pred_val = network(on_gpu.unsqueeze(0)).item()
                running_point_average.append(pred_val)
            
            average_prediction = sum(running_point_average) / len(running_point_average)
            
            #Assemble list of looked-up nearest neighbor values from ranked list
            sq_error = (act_val - average_prediction)**2
            ppe_list.append(sq_error)
...
def single_file_neigh_absolute_test(network,
                                    dataset,
                                    curr_test_file,
                                    prediction_mode,
                                    num_possible_neighbors):
    ...
    for possible_point in dataset.full_test_getitem:
        if possible_point[0] == curr_test_file:
            ...
            for specific_pred in range(num_possible_neighbors):
                #Assemble list of looked-up nearest neighbor values from ranked list
                #Here is where to apply uniform or boltzmann averaging
                #Looking for minimum predicted error, so <= rather than >=
                key_string = 'num_neigh=' + str(specific_pred + 1)
                target_list = [train_list[ranks.index(rank)] if rank <= specific_pred else 0.0 for rank in ranks]
                if prediction_mode == 'uniform':
                    total_pred = sum(target_list)
                    avg_pred = total_pred / (specific_pred + 1)
                    sq_error = (act_val - avg_pred)**2
                    per_point_result_dict[key_string].append(sq_error)
